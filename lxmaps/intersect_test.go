...
func TestIntersect_StringInt(t *testing.T) {
	tests := []struct {
		name     string
		m1       map[string]int
		m2       map[string]int
		...
		{
			name:     "multiple common keys, m2 has extra",
			m1:       map[string]int{"a": 1, "b": 2, "c": 3},
		...
		},
		{
			...
			m2:       map[string]int{"x": 999, "y": 888, "z": 777},
			wantNil:  false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := lxmaps.Intersect(tt.m1, tt.m2)
...
func TestIntersect_IntString(t *testing.T) {
	tests := []struct {
		name     string
		m1       map[int]string
		m2       map[int]string
	...
	}{
		{
			...
			expected: map[int]string{2: "b", 3: "c"},
		},
		{
			name:     "negative keys",
			m1:       map[int]string{-1: "a", 0: "b", 1: "c"},
			m2:       map[int]string{-1: "x", 0: "y"},
...
func TestIntersect_BoolFloat64(t *testing.T) {
	tests := []struct {
		name     string
		m1       map[bool]float64
		m2       map[bool]float64
	...
	}{
		{
			...
			expected: map[bool]float64{true: 1.5},
		},
		{
			name:     "only false match",
			m1:       map[bool]float64{true: 1.1, false: 2.2},
			m2:       map[bool]float64{false: 3.3},
			expected: map[bool]float64{false: 2.2},
		},
	}
...
func TestIntersect_DoesNotModifyInputs(t *testing.T) {
	m1 := map[string]int{"a": 1, "b": 2}
...
func TestIntersect_StructValue(t *testing.T) {
	type Person struct {
		Name string
		Age  int
	}
...
func TestIntersect_UnicodeKeys(t *testing.T) {
	tests := []struct {
		name     string
		m1       map[string]int
		m2       map[string]int
...
func TestIntersect_SpecialCharKeys(t *testing.T) {
	tests := []struct {
		name     string
		m1       map[string]int
		m2       map[string]int
...
func TestIntersect_CaseSensitive(t *testing.T) {
	m1 := map[string]int{
		"Key": 1,
		"key": 2,
		"KEY": 3,
...
func TestIntersect_LargeMap(t *testing.T) {
	...
	// Should have approximately 1/3 of entries
	expectedCount := 10000 / 3
	if result == nil {
